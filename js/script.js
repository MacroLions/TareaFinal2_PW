...
function getAge(birthday) {
    ...
    var ageDifMs = presentDate - birthday + leapYears();
    var ageDate = new Date(ageDifMs); // miliseconds from epoch
    var day = presentDate.getUTCDay();
    var bday = Math.abs(ageDate.getUTCDay());
    var month = presentDate.getUTCMonth();
    var bmonth = Math.abs(ageDate.getUTCMonth());
...
function leapYears(){
    let yearStart = 1;
    let yearEnd = 1970;
    for(var cont = 0; yearStart <= yearEnd; yearStart++){
        if((((yearStart%4)!= 0) && ((yearStart%100) == 0)) || ((yearStart%400) == 0)){
            cont++
        }
    }return (cont)*24*Math.pow(60, 2)*1000;
}